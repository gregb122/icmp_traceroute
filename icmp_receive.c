...
int receive_with_ttl(int _sockfd, int _ttl, struct timeval* _tv, int _number_ofpings, uint8_t _buffer[],
	...
	for (;;){
		...
		if (ready == 0){
			return 3; // Nie mamy nic gotowego do odczytania
		}
        ...
        if(icmp_header->icmp_type == ICMP_TIMXCEED){
            uint8_t* start_of_orig_data = _buffer + ip_header_len + sizeof(icmp_header);
            ip_header     = (struct ip*)(start_of_orig_data);
            ip_header_len = 4 * ip_header->ip_hl;
            icmp_header = (struct icmp*)(start_of_orig_data + ip_header_len);
        }
		
        if(icmp_header->icmp_hun.ih_idseq.icd_id != getpid() 
		|| icmp_header->icmp_hun.ih_idseq.icd_seq/_number_ofpings != _ttl-1){
            continue;
        }
