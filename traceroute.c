...
int send_pings(int _sockfd, struct sockaddr_in _recipient, int _ttl, int _number_of_pings){
    ...
    for (int i = _number_of_pings; i > 0; i--){
        out = send_ping(_sockfd, _recipient, _ttl, _ttl); //(_ttl - 1)*_number_of_pings + 1) ale my nie musimy rozrózniac
...
void print_ips(long* numbers, char ip_addresses[][20], int count){
...
int receive_pings(int _sockfd, int _ttl, int _number_of_pings)
{
    ...
    long times_of_answers[_number_of_pings];
    memset(sender_ip_str, 0, sizeof(sender_ip_str[0][0]) * 3 * sizeof(sender_ip_str[0]));
    int get_atleast_one = 0, get_to_dest_host = 0;
    for(int i = 0; i < _number_of_pings; i++){
        int out = receive_with_ttl(_sockfd, _ttl, &tv, buffer, sender_ip_str[i]);
        ...
        if (out == 2){
            ...
            times_of_answers[i] = tv.tv_usec ? (1000000 - tv.tv_usec) / 1000 : 0;
            get_atleast_one = 1;
        } else {
            //jesli nie ustawiamy czas tego jednego pakietu na -1
            times_of_answers[i] = -1;
        }
    }

    if(get_atleast_one){
        long avr = avrg(times_of_answers, _number_of_pings);
        print_ips(times_of_answers, sender_ip_str, _number_of_pings);
...
int traceroute(char* _receiver_ip){
    ...
    for (int ttl=1; ttl <= 30; ttl++){
        ...
        recv_out = receive_pings(sockfd, ttl, 3);
    }
    return EXIT_FAILURE;
}
